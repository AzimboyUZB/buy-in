...
import { cartAddUrl, cartListUrl, GetcartProductRemoveUrl } from './../../helpers/url'

const SET_LOADING = 'SET_LOADING'
const SET_CART_PRODUCTS = 'SET_USER'
...
export function AddProduvtToCart(product_id, qty = 1) {
    return async function (dispatch) {
        Axios.post(cartAddUrl,{ qty, product_id}, )
            .then(function (respons) {
                if(respons.data.cart){
                    dispatch(SetCartProductsAC(respons.data.cart))
                }
                console.log(respons);
            }
...
export function RemoveCartProduct(product_id) {
    return async function (dispatch) {
        ...
        Axios.post(GetcartProductRemoveUrl(product_id)).then(function (response) {
            if (response.data.cart) {
                dispatch(SetCartProductsAC(response.data.cart))
                dispatch(SetLoadingAC(false))
            }
